...
struct btree {
  // num_keys is the number of keys in array that are currently valid.
  int num_keys;
  ...
  // children is an array of child pointers to b-tree subtrees. valid
...
public:
  ...
  // helper function to remove key from a leaf node
  void remove_from_leaf(shared_ptr<btree> root, int i);

  // helper function to remove key from a non-leaf node
  void remove_from_nonleaf(shared_ptr<btree> root, int i, int key);
...
private:
  ...
  void split(shared_ptr<btree> root); 
};
